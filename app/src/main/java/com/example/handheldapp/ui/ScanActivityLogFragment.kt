...
/**
 * Fragment untuk menampilkan RIWAYAT LENGKAP scan (setiap scan = 1 baris)
 */
@AndroidEntryPoint
class ScanActivityLogFragment : Fragment() {
    ...
    private fun observeData() {
        // Observe RIWAYAT LENGKAP data (tanpa GROUP BY)
        viewModel.scanActivityLog.observe(viewLifecycleOwner) { resource ->
            when (resource) {
                ...
                is Resource.Error -> {
                    showLoading(false)
                    android.util.Log.e("ScanActivityLog", "❌ Error: ${resource.message}")
                    showEmptyState(true)
                }
            }
        }
    ...
    companion object {
        fun newInstance() = ScanActivityLogFragment()
    }
}
